...
class Venues extends AdminController
{
    ...
    public function venues_list()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $limit = $this->input->get('limit') ? $this->input->get('limit', true) : 10;
            $offset = $this->input->get('offset') ? $this->input->get('offset', true) : 0;
            $order_type = $this->input->get('order_type') ? $this->input->get('order_type', true) : 'DESC';
            $order_by = $this->input->get('order_by') ? $this->input->get('order_by', true) : 'venue_id';
            $search = $this->input->get('search') ? $this->input->get('search', true) : '';
            ...
            header('Content-Type: application/json');
            echo json_encode(array('rows' => $venues, 'total' => $total));
            die();
        }
        $this->_render_page();
    }
